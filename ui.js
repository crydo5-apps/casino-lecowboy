...
export function renderGrid(grid, options = {}) {
  ...
  grid.forEach((row,r) => row.forEach((item,c) => {
    ...
    if (SYMBOL_ASSETS[assetKey]) { symbol.classList.add('has-art'); symbol.style.setProperty('--symbol-art',`url("${SYMBOL_ASSET_ROOT}${SYMBOL_ASSETS[assetKey]}")`); }
...
export function showPaytable() {
  ...
  items.forEach(([id,name])=>{const item=document.createElement('div');item.className='pay-card';const icon=document.createElement('span');icon.className=`pay-symbol ${id}`;icon.textContent=id==='10'?'10':symbolLabel({kind:'regular',id});icon.setAttribute('aria-label',icon.textContent);if(SYMBOL_ASSETS[id]){icon.classList.add('has-art');icon.style.setProperty('--symbol-art',`url("${SYMBOL_ASSET_ROOT}${SYMBOL_ASSETS[id]}")`);}const label=document.createElement('span');label.textContent=name;const value=document.createElement('strong');value.textContent=id==='wild'||id==='scatter'?'FEATURE':id==='10'?'0.2×–50×':id==='badge'?'1×–200×':id==='hat'||id==='cactus'?'0.4×–75×':'0.6×–100×';item.append(icon,label,value);grid.appendChild(item);});
